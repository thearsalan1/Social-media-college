...
export const reportContent = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!Object.values(TargetTypeEnum).includes(targetType)) {
      return res.status(400).json({ success: false, message: "Target type invalid" });
    }
    ...
    if (target.reportedBy.includes(userId)) {
      return res.status(400).json({ success: false, message: "You already reported this" });
    }
    ...
    if (target.reportCount >= REPORT_THRESHOLD) {
      ...
      await emitNotifications({
        ...
        recipientId: "ADMIN",   // ⚠️ note neeche dekh
      ...
      logger.info("Content auto-hidden due to reports", { targetType, targetId, reportCount: target.reportCount });
    }
...
export const unbanContent = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: "Content restored", data: target });
  } catch (error) {
